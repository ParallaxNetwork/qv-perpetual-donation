 ...
 */
bot.start((ctx: any) => {
  console.log("[DEBUG] User started the bot", ctx);
  ...
  console.log("[DEBUG] User data extracted from the context", userData);

  // Generate the hash for Telegram authentication
  const hash = generateTelegramHash(userData);

  console.log("[DEBUG] Hash generated for user", hash);
